...
- Adds a non-explicit domestic company action with prestige, relationship, and savings effects plus cooldown. Owner-slave pregnancy mechanics are intentionally not implemented; romance and bastard mechanics remain for free consenting adult relationships, manumitted characters, and the existing lover system.
...
Household slave tasks are intentionally modest for balance and have task-change cooldowns. They create useful owner benefits without infinite stacking: cash from accounts/labor, prestige from entertainment/domestic company, influence from guards, directed child skill improvement from educators, and possible treatment from doctors. Owned slaves also accumulate small savings toward self-purchase. Society does not implement owner-slave sexual coercion or owner-slave pregnancy mechanics; slave family growth is handled through spouse links between owned slaves, while romance and bastard mechanics remain limited to free adult relationship systems.
