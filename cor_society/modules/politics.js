{
  ...
  methods: {
    _mixinCorSocietyPolitics() {
      ...
      if (window.corSociety._mixinCorSocietyPoliticsVersion === '1.1.325') {
        return
      }
      Object.assign(window.corSociety, {
        ...
        openPolitics() {
                  ...
                  let summary = [
                    ...
                    this.summaryOption('Ruler', isEmperor ? 'You ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Imperator' : isDictator ? 'You ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Dictator' : (rulerHouse ? rulerHouse.name : (rome.government === 'republic' ? 'the Senate' : 'none')), [isEmperor ? this.imperatorIcon() : this.affairIcon('prestige')], 'Who currently holds supreme power.'),
                  ...
                  this.pushModal({
                    ...
                    message: rome.government === 'republic'
                      ? 'The Republic stands. Climb the cursus honorum, build a faction, and ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â in a crisis ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you may be named Dictator. Few dictators ever become Emperor.'
        ...
        openFactionRecruit() {
                  ...
                  let options = houses.map((house) => {
                    ...
                    return this.politicsButton('recruitFactionHouse', (inFaction ? 'ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ ' : '') + house.name + ' (relation ' + this.signed(house.relation || 0) + ', cost ' + cost + ')', 'support', { houseId: house.id }, inFaction || cash < cost ? { disabled: true, showDisabledWithTooltip: true, tooltip: inFaction ? 'Already in your faction.' : 'Not enough cash (' + cost + ').' } : {})
                  })
        ...
        openHousePolitics({ houseId } = {}) {
                  ...
                  let propSummary = props.length ? props.slice(0, 5).map((p) => p.count + 'ÃƒÆ’Ã¢â‚¬â€ ' + p.title).join(', ') : 'no recorded estates'
                  let options = []
                  options.push(this.politicsButton('pressLandClaim', 'Press a land claim ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â seize a property (' + this.costLabel(claimCost) + ')', 'coins', { houseId }))
        ...
        openSeizeProperty({ houseId } = {}) {
                  ...
                  let options = props.map((p) => {
                    return this.politicsButton('seizeProperty', 'Seize ' + p.title + ' (ÃƒÆ’Ã¢â‚¬â€' + p.count + ', worth ' + p.value + ' each)', p.group === 'animal' ? 'familyTree' : 'coins', { houseId, key: p.key })
                  })
                  if (!options.length) {
                  ...
                  this.pushModal({
                    ...
                    message: 'Your claim succeeds. Choose what to take from ' + house.name + ' ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a real property passes into your house. (Animals can be rustled too.)',
                    image: this.affairIcon('coins'),
                    options
                  })
                },
        ...
        openAbductTargets({ houseId } = {}) {
                  ...
                  // never random ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â pick exactly who to abduct.
                  let memberIds = []
        ...
        openDemandProperty({ captiveId } = {}) {
                  ...
                  let props = this.housePropertyOptions(house)
                  let options = props.map((p) => {
                    return this.politicsButton('demandProperty', 'Demand ' + p.title + ' (ÃƒÆ’Ã¢â‚¬â€' + p.count + ', worth ' + p.value + ')', p.group === 'animal' ? 'familyTree' : 'coins', { captiveId, key: p.key })
                  })
        ...
        dictatorshipGate(society, state) {
                  ...
                  let propertyValue = house && this.housePropertyValue ? this.housePropertyValue(house) : 0
                  if (faction < 3) {
                    return { ready: false, reason: 'Win more houses to your faction ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â at least 3 must stand behind you (have ' + faction + ').' }
                  }
                  if (auctoritas < 70) {
                  ...
                  if (cash < 250 && propertyValue < 3000 && influence < 1500) {
                    return { ready: false, reason: 'A dictator must have means ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â greater wealth (250+ cash or 3000+ in estates) or far more influence ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â before the Senate would trust you with imperium.' }
                  }
        ...
        dictatorMandateList() {
                  ...
                  return [
                    { id: 'rei_gerundae', title: 'Rei gerundae causa ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â to wage a war', term: 6, progress: 0, note: 'War powers in a military crisis. Honourable, but a dead end for ambition.' },
                    { id: 'seditionis', title: 'Seditionis sedandae causa ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â to crush sedition', term: 6, progress: 2, note: 'Emergency powers against unrest at home.' },
                    { id: 'comitiorum', title: 'Comitiorum habendorum causa ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â to hold the elections', term: 2, progress: -4, note: 'A short, narrow mandate; little room to build power.' },
                    { id: 'legibus', title: 'Legibus scribundis et rei publicae constituendae ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â to remake the Republic (Sulla)', term: 11, progress: 14, note: 'A long legislative mandate. Lets you LEGISLATE ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the true road toward the purple.', powerLaws: true }
                  ]
                },
        openDictatorMandates() {
                  ...
                  this.pushModal({
                    ...
                    message: 'The Senate names a Dictator only for a stated purpose and a fixed term. Choose your causa ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â only a Sullan legislative mandate truly opens the road to the purple.',
                    image: this.affairIcon('senator'),
                    options
                  })
                },
        seekDictatorship({ mandate } = {}) {
                  ...
                  if (Math.random() < chance) {
                    ...
                    message = 'A consul, on the Senate\'s authority, names you DICTATOR ' + chosen.title.split(' ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ')[0] + ' for a ' + chosen.term + '-month term, with a master of horse (magister equitum) at your side.' + (chosen.powerLaws ? ' Your legislative mandate lets you remake the laws ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the road to the purple is open.' : ' This is a limited mandate; lay it down with honour when the task is done.')
        ...
        attemptEmpire() {
                  ...
                  if (!failedStage) {
                    ...
                    message = 'After Dictatorship comes the purple. The Senate, army, and people acclaim you IMPERATOR ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â and the eagle of Rome (SPQR) is raised in your name. Rome becomes an Empire. War-time edicts and your imperial powers now ripple across every house, and the imperial fisc pays you each month.'
                  } else {
                    // Failure branches ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â costly, sometimes catastrophic.
                    ...
                    if (roll < 0.45) {
                      ...
                      message = 'You fail at: ' + failedStage + '. You are proscribed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â your property plundered and your name dragged through the courts.'
        ...
        openEmperorActions() {
                  ...
                  this.pushModal({
                    societyMenu: true,
                    title: 'Imperator ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Imperial powers',
                    message: 'As Imperator, under the eagle of Rome (SPQR), you command appointments, war, and the loyalty of Rome, and the imperial fisc fills your treasury each month. Rivals will push to change the succession to favour the wealthiest or strongest house ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â hold them off.',
        ...
        openAppointGovernor() {
                  ...
                  this.pushModal({ societyMenu: true, title: 'Appoint a governor', message: 'Reward a house with a province. It strengthens their loyalty and revenue ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â and your patronage network.', image: this.affairIcon('senator'), options })
                },
        ...
        pathToPowerHint(society, state) {
                  let rome = this.ensureRomeState(society)
                  if (this.playerIsEmperor(society, state)) {
                    return 'You rule as Imperator. Defend the succession and project power ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â rivals will push to change the law.'
                  }
                  if (this.playerIsDictator(society, state)) {
                    let egate = this.empireGate(society, state)
                    ...
                    return 'Hint: to reach the purple ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ' + egate.reason + ' As Dictator, propose LAWS to build legislative progress (' + prog + '%) toward the throne.'
                  }
                  if (rome.government === 'republic') {
                    let dgate = this.dictatorshipGate(society, state)
                    if (dgate.ready) {
                      return 'Hint: you can seek the Dictatorship now ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the required first step toward becoming Emperor.'
                    }
                    return 'Hint: climb toward the Dictatorship first ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ' + dgate.reason
                  }
                  return ''
                },
        becomeEmperor(society, state) {
                  ...
                  this.log(society, 'You become Imperator ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Emperor of Rome.', 'prestige')
                },
        lawList() {
                  return [
                    { id: 'lex_de_imperio', title: 'Lex de Imperio ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â grant yourself greater imperium', cost: { influence: 40 }, progress: 12, auctoritas: 10, oppose: 3, civilWar: 0 },
                    { id: 'pack_senate', title: 'Pack the Senate with your clients', cost: { cash: 120 }, progress: 14, auctoritas: 12, oppose: 4, civilWar: 0 },
                    { id: 'extend_term', title: 'Dictator legibus scribundis ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â extend your term', cost: { influence: 60 }, progress: 18, auctoritas: 16, oppose: 6, civilWar: 0.08, extendTerm: 8 },
                    { id: 'perpetual', title: 'Dictator perpetuo ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â dictator for life (Caesar)', cost: { influence: 100, cash: 150 }, progress: 30, auctoritas: 24, oppose: 10, civilWar: 0.45 },
                    { id: 'march', title: 'March your clients on Rome ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â force the issue (civil war)', cost: { cash: 200 }, progress: 0, auctoritas: 0, oppose: 12, civilWar: 1 }
                  ]
                },
        ...
        policyLawList() {
                  return [
                    ...
                    { id: 'lex_agraria', title: 'Lex Agraria ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â redistribute the great estates', cost: { influence: 55 }, populist: 6, hurt: { groups: ['land', 'estate'], who: 'the great landowners' }, help: { groups: ['animal'], who: 'smallholders & plebs' } },
                    { id: 'lex_claudia', title: 'Lex Claudia ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â bar senators from sea trade', cost: { influence: 40 }, populist: 1, hurt: { groups: ['boat'], who: 'the shipping houses' }, help: { groups: ['land'], who: 'the landed houses' } },
                    { id: 'lex_frumentaria', title: 'Lex Frumentaria ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â grain dole for the people', cost: { cash: 160 }, populist: 8, allRelation: 4, hurt: false, help: false },
                    { id: 'novae_tabulae', title: 'Novae Tabulae ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â cancel debts', cost: { influence: 50 }, populist: 5, hurt: { wealthAbove: 600, who: 'the great creditors' }, help: { wealthBelow: 220, who: 'indebted houses' } },
                    { id: 'vectigal_portus', title: 'Harbour dues ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â tax the trading fleets', cost: { influence: 25 }, populist: 0, hurt: { groups: ['boat'], who: 'trading fleets' }, help: { groups: ['land', 'animal'], who: 'farmers & herders' } }
                  ]
                },
        ...
        triggerCivilWar(society, state, law) {
                  ...
                  if (Math.random() < winChance) {
                    this.becomeEmperor(society, state)
                    return 'CIVIL WAR! Your legions and clients prevail. On the ashes of the Republic you are acclaimed IMPERATOR ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Rome is now an Empire.'
                  }
                  rome.government = 'republic'
                  rome.dictatorHouseId = ''
                  rome.dictatorTermEnd = ''
                  ...
                  // character dies here ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the base game then continues the player with their heir.
                  ...
                  if (executed) {
                    ...
                    return 'CIVIL WAR! Your legions are crushed and YOU are executed as a traitor. Your house fights on ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â you continue as your heir.'
                  }
                  ...
                  return 'CIVIL WAR! Your forces are crushed. You are driven from power, proscribed, and the Republic ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â battered ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â endures.'
                },
        imperialCrimes() {
                  return [
        ...
        openImperialActions({ houseId, characterId } = {}) {
                  ...
                  this.pushModal({
                    societyMenu: true,
                    title: 'Imperial justice ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ' + this.characterName(character, state),
                    message: inquiry ? 'The inquiry found: ' + (crime ? crime.label.toUpperCase() : 'nothing ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â they appear loyal') + '.' : 'As Imperator you hold cognitio over every Roman. Order an inquiry first, then judge.',
                    image: this.imperatorIcon(),
                    options
                  })
                },
      ...
      window.corSociety._mixinCorSocietyPoliticsVersion = '1.1.325'
    }
  }
}
