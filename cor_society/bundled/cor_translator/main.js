{
  ...
  checkAndAct() {
    daapi.invokeMethod({
      event: '/cor_society/bundled/cor_translator/main',
      method: 'boot'
    })
    daapi.addGlobalAction({
      ...
      action: {
        ...
        icon: daapi.requireImage('/cor_society/bundled/cor_translator/icon.svg'),
        isAvailable: true,
        process: {
          event: '/cor_society/bundled/cor_translator/main',
          method: 'openSettings'
  ...
  methods: {
    boot() {
      if (window.corTranslator && window.corTranslator.version === '1.0.6') {
      ...
      window.corTranslator = {
        ...
        showInstallNoticeOnce() {
          ...
          setTimeout(() => {
            daapi.pushInteractionModalQueue({
              ...
              image: daapi.requireImage('/cor_society/bundled/cor_translator/icon.svg'),
              options: [
                {
                  variant: 'info',
                  text: 'Open settings',
        ...
        fetchTranslationOnce(key) {
          // Single, de-duplicated network path for every caller (modal pretranslate AND the scan
          // queue). If the same text is already cached, already failed recently, or already being
          // fetched, we never send a second request for it. Returns the raw cached translation
          // (not formatted), or '' when unavailable.
          key = this.normalizeText(key)
          if (!this.shouldTranslateRaw(key)) {
            return Promise.resolve('')
          }
          if (this.dictionary[key]) {
            return Promise.resolve(this.dictionary[key])
          }
          if (!this.canFetchTranslations()) {
            return Promise.resolve('')
          }
          if (this.failedAt[key] && Date.now() - this.failedAt[key] < 300000) {
            return Promise.resolve('')
          }
          ...
          let promise = this.withTimeout(this.translateText(key), 3500).then((translated) => {
        ...
        translateAndCacheNow(key) {
          ...
          return this.fetchTranslationOnce(key).then((translated) => {
            return translated ? this.formatTranslation(key, translated) : ''
          })
        },
        ...
        startObserver() {
          ...
          this.observer = new MutationObserver(() => {
            // Coalesce DOM mutations into an occasional scan. Applying a translation only mutates a
            // node when its text actually changes, so a stable screen produces no further scans;
            // during active learning this throttle keeps scans from stampeding.
            let sinceLastScan = Date.now() - (this.lastScanAt || 0)
            this.scheduleScan(sinceLastScan < 250 ? 250 : 120)
          })
        ...
        pumpQueue() {
          ...
          // Route through the shared, de-duplicated fetch so the queue can never request a text
          // that a modal pretranslation is already fetching (or vice versa). Caching, stats, error
          // backoff and saving are all handled inside fetchTranslationOnce.
          this.fetchTranslationOnce(text)
            .then((translated) => {
              if (translated) {
    ...
    openSettings() {
      if (!window.corTranslator) {
        daapi.invokeMethod({ event: '/cor_society/bundled/cor_translator/main', method: 'boot' })
      }
